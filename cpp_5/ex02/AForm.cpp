...
AForm::AForm(std::string &name, std::string &target, int sign, int execute) : _name(name), _target(target), _status(0), _sign(sign), _execute(execute) {
	if (sign < 1 || execute < 1)
...
const char *AForm::GradeTooLowException::what() const throw() {
	return ("Bureaucrat don't have the grade to sign this Aform.");
}
